...
from utils.config import SHEA_BLUE, SHEA_GOLD, SHEA_DARK, POS_GREEN, NEG_RED
...
def render(df, fdf, page):
    ...
    _pct_high = (fdf["total_score"] >= 4).mean()
    _at_risk_n = (fdf["total_score"] <= 3).sum()
    _at_risk_pct = (fdf["total_score"] <= 3).mean()
